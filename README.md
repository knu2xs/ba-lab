...
Potentially useful and completely unsupported ideas.
...
```
    ...
    ├── setup.py           <- Setup script for the library (ba_lab)
    ...
    │   ├── ba-lab.aprx <- ArcGIS Pro project.    
    │   └── ba-lab.tbx  <- ArcGIS Pro toolbox associated with the project.
    ├── data
    ...
    └── src                <- Source code for use in this project - all scripts, modules and code.
        └── ba_lab <- Library containing the bulk of code used in this 
                                                  project. 
```
